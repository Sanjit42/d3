...
var sortByMethod = function (title, method) {
    if (filteredValue=='' || !filteredValue) {
        ...
        showUserValue('.container', {title: title + ' values are:-', method: filteredValue.sort(method)});
    }
};
...
var sortByScan = function () {
    if (filteredValue == '' || !filteredValue) {
        ...
        showUserValue('.container', {title: 'Scaned value is:-', method: scanValues});
    }
};
var getBisector = function () {
    if (!bisectValue) {
        ...
        showUserValue('.container', {title: 'Scaned value is:-', method: index});
    }
};
...
var dataStore = function () {
    ...
    if (filteredValue == '') {
        ...
        d3.select('.value').html("");
        showUserValue('.value', {title: 'Entered numbers are :- ', method: filteredValue});
    }
...
var bisectorData = function () {
    ...
    if (bisectValue != "") {
        ...
        showUserValue('.bisectorValue', {title: 'Entered quantile value is :- ', method: bisectValue});
    } else {
        alert('please enter bisector value');
        d3.select('.bisectorValue').html("");
    }
